...
Scale:
* The SCALE constant must be set in your own *.scad file since variables cannot be reassigned at run time. This allows creation of both lego and duplo compatible bricks in one run.

Normal size (DUPLO size)
// SCALE = 1;
...
Must be adjusted with respect to layer resolution and other slicing considerations. This is crucial for fit.
Real DUPLO outer radius = 13.48
Real Lego outer radius = 6.5
*/
function NB_BOTTOM_RADIUS(SCALE)        = (SCALE < 0.6) ? 6.2/2*LEGO_SCALE(SCALE) : 13.4/2*SCALE; // cheat
function NB_BOTTOM_RADIUS_THIN(SCALE)   = (SCALE < 0.6) ? 3.5/2*LEGO_SCALE(SCALE) : 7.2/2*SCALE;

function NB_BOTTOM_RADIUS_INSIDE(SCALE) = (SCALE < 0.6) ? 4.9/2*LEGO_SCALE(SCALE) : 10.8/2*SCALE;
// Real DUPLO inner r= 10.73
// Real Lego inner r = 4.9 the hole is not really needed, so if it is too small it will not matter
