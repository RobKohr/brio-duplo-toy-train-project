...
module doblo (col=0, row=0, up=0, width=4,length=2,height=FULL,
	      nibbles_on_off=true,diamonds_on_off=false,scale=LUGO,orientation=0, lattice_type=LATTICE_TYPE, shave=SHAVE) 
     ...
     // adds a bottom lattice that extends down. Sturdier but longer to print
{
  ...
  translate ([0, y_start, z_pos]) {
       ...
       for (i = [0:length-2]) {
	    ...
	    { cube([width*PART_WIDTH(scale)-nudge-shave, LATTICE_WIDTH(scale), height*PART_HEIGHT(scale)-nudge],true); }
       }
  }
  // grid along x-axis
  translate ([x_start, 0, z_pos]) {
       ...
       for (j = [0:width-2]) {
	    ...
	    { cube([LATTICE_WIDTH(scale),length*PART_WIDTH(scale)-nudge-shave,height*PART_HEIGHT(scale)-nudge],true); }
       }
  }
}
...
module bottom_lattice (width, length, height, scale, lattice_type, shave)
{
  ...
  translate ([0, y_start, z_pos]) {
    ...
    for (i = [0:lattice_type*length-2]) {
      translate([0, i* spacing, 0])
      { cube([width*PART_WIDTH(scale)-shave-NUDGE, LATTICE_WIDTH(scale), LATTICE_WIDTH(scale)],true); }
    }
  }
  // grid along x-axis
  translate ([x_start, 0, z_pos]) {
    ...
    for (j = [0:lattice_type*width-2]) {
      translate([j * spacing, 0, 0]) 
      { cube([LATTICE_WIDTH(scale),length*PART_WIDTH(scale)-shave-NUDGE,LATTICE_WIDTH(scale)],true); }
    }
  }
}
