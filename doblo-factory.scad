/*
 ...
 Copyright (c) 2017/2018 Daniel K. Schneider
 
 This file is part of DobloFactory VERSION 2.4

Read file README and http://edutechwiki.unige.ch/en/Doblo_factory
...
// You may change this include based on printer types
// include <lib/doblo-params.scad>; // uses (mostly) CORRECT REAL VALUES, will not work well with FDM slicers and technology.
include <lib/doblo-params-felixtec4.scad>; // A modern fast PLA printer, slightly smaller walls
  ...
  */
{
  ...
  // User info
  echo(str("<font color='green'>DOBLO brick</font>. width(x)=", width * PART_WIDTH(scale), "mm, length=", length * PART_WIDTH(scale), "mm, height=", height * PART_HEIGHT(scale), "mm", ", Shave fix=", shave, "mm"));

  // echo ("Orientation points =", col*PART_WIDTH(scale),row*PART_WIDTH(scale));
  // orientation (rotation) is around the upper left corner, i.e. col and row
      ...
      union() {
        ...
        if (scale > 0.3) {
          if (width > 1 && length > 1) {
            ...
            if (lattice_type > 0 && lattice_type < 3) {
              color("green") bottom_lattice(width, length, height, scale, lattice_type, shave);
            } else if (lattice_type == 3) {
              color("green") full_bottom_lattice(width, length, height, scale, lattice_type, shave);
            }
          } else {
            // big nibbles underneath
            if (USE_INSET(scale))
              bottom_nibbles_part(width, length, height, scale);
          }
          if ( (scale < 0.6) && (!USE_INSET(scale))) {
            bottom_nibbles_thin(width, length, height, scale);
          }
          ...
          if (USE_INSET(scale))
            difference() {
              union() {
                for (j = [1:length])
                  translate([0, -PART_WIDTH(scale) * length / 2 + j * 2 * NO(scale) - 2 * NO(scale) / 2, 0]) cube([width * PART_WIDTH(scale) - shave, INSET_WIDTH(scale), height * PART_HEIGHT(scale)], true);
                for (i = [1:width])
                  translate([-PART_WIDTH(scale) * width / 2 + i * 2 * NO(scale) - 2 * NO(scale) / 2, 00]) cube([INSET_WIDTH(scale), length * PART_WIDTH(scale) - shave, height * PART_HEIGHT(scale)], true);
              }
...
module bottom_lattice_wide(width, length, height, scale, bottom_lattice, shave) {
  x_start = -width / 2 * PART_WIDTH(scale) + NBO(scale);
  y_start = -length / 2 * PART_WIDTH(scale) + NBO(scale);
  ...
  translate([0, y_start, z_pos]) {
    // grid along y-axis
    for (i = [0:length - 2]) {
      translate([0, i * NBO(scale), 0]) { cube([width * PART_WIDTH(scale), LATTICE_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
    }
  }
  // grid along x-axis
  translate([x_start, 0, z_pos]) {
    // holes along x-axis
    for (j = [0:width - 2]) {
      translate([j * NBO(scale), 0, 0]) { cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
    }
  }
}

module full_bottom_lattice(width, length, height, scale, lattice_type, shave)
// adds a bottom lattice that extends down. Sturdier but longer to print
{
  nudge = 0.1;
  spacing = NBO(scale);
  ...
  translate([0, y_start, z_pos]) {
    // grid along y-axis
    for (i = [0:length - 2]) {
      translate([0, i * spacing, 0]) { cube([width * PART_WIDTH(scale) - nudge - shave, LATTICE_WIDTH(scale), height * PART_HEIGHT(scale) - nudge], true); }
    }
  }
  // grid along x-axis
  translate([x_start, 0, z_pos]) {
    // holes along x-axis
    for (j = [0:width - 2]) {
      translate([j * spacing, 0, 0]) { cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale) - nudge - shave, height * PART_HEIGHT(scale) - nudge], true); }
    }
  }
}

module bottom_lattice(width, length, height, scale, lattice_type, shave) {
  ...
  translate([0, y_start, z_pos]) {
    ...
    for (i = [0:lattice_type * length - 2]) {
      translate([0, i * spacing, 0]) { cube([width * PART_WIDTH(scale) - shave - NUDGE, LATTICE_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
    }
  }
  // grid along x-axis
  translate([x_start, 0, z_pos]) {
    ...
    for (j = [0:lattice_type * width - 2]) {
      translate([j * spacing, 0, 0]) { cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale) - shave - NUDGE, LATTICE_WIDTH(scale)], true); }
    }
  }
}

module bottom_grid(width, length, height) {
  spacing = NBO(scale) / lattice_type;
  x_start = -width / 2 * PART_WIDTH(scale) + spacing;
  y_start = -length / 2 * PART_WIDTH(scale) + spacing;
  z_pos = height * PART_HEIGHT(scale) / 2 - LATTICE_WIDTH(scale) - LATTICE_WIDTH(scale) / 2;
  translate([0, y_start, z_pos]) {
    // grid along y-axis
    for (i = [0:lattice_type * length - 2]) {
      translate([0, i * spacing, 0]) { cube([width * PART_WIDTH(scale), LATTICE_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
    }
  }
  // grid along x-axis
  translate([x_start, 0, z_pos]) {
    // holes along x-axis
    for (j = [0:lattice_type * width - 2]) {
      translate([j * spacing, 0, 0]) { cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
    }
  }
}
...
module bottom_nano_lattice(width, length, height) {
  ...
  translate([0, y_start, z_pos]) {
    // grid along y-axis
    for (i = [0:LATTICE_TYPE * length - 2]) {
      translate([0, i * spacing, 0]) { color("green") cube([width * PART_WIDTH(scale), LATTICE_WIDTH(scale), height * PART_HEIGHT(scale)], center=true); }
    }
  }
  // grid along x-axis
  translate([x_start, 0, z_pos]) {
    // holes along x-axis
    for (j = [0:LATTICE_TYPE * width - 2]) {
      translate([j * spacing, 0, 0]) { color("blue") cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale), height * PART_HEIGHT(scale)], center=true); }
    }
  }
}
...
module block(col = 0, row = 0, up = 0, width = 2, length = 4, height = FULL, nibbles_on_off = false, scale = LUGO, orientation = 0, debug = false, shave = SHAVE)
/* Use cases:
   ...
   */
{
  ...
  z_0 = up * PART_HEIGHT(scale) + height * PART_HEIGHT(scale) / 2.0;
  if (debug)
    echo(str("<font color='lightgreen'>DOBLO block</font>. Width(left/right,x)=", width * PART_WIDTH(scale), "mm, length(forth/back,y)=", length * PART_WIDTH(scale), "mm, height=", height * PART_HEIGHT(scale), "mm"));
  // the cube is drawn at absolute x,y,z = 0 then moved
  rotate_about_z(angle=orientation, point=[col * PART_WIDTH(scale), -row * PART_WIDTH(scale)]) // row is inverted
    translate([x_0, y_0, z_0]) {
      //the cube
    ...
    union() {
      translate([x_0, y_0, z_pos]) {
        translate([0, y_start, 0]) {
          // grid along y-axis
          for (i = [0:n_rows]) {
            translate([0, i * spacing, 0]) { cube([width * PART_WIDTH(scale) - offset * 2, LATTICE_WIDTH(scale), LATTICE_WIDTH(scale)], true); }
          }
        }
        // grid along x-axis
        translate([x_start, 0, 0]) {
          // holes along x-axis
          for (j = [0:n_cols]) {
            translate([j * spacing, 0, 0]) { cube([LATTICE_WIDTH(scale), length * PART_WIDTH(scale) - offset * 2, LATTICE_WIDTH(scale)], true); }
          }
        }
      }
    }
...
// thanx to Mark Harrison
// https://stackoverflow.com/questions/45826208/openscad-rotating-around-a-particular-point
// point is an array [x,y,z]

// rotate as per a, v, but around point pt
module rotate_about_pt(angle, rot_vector, rot_pt) {
  translate(rot_pt)
    rotate(angle, rot_vector)
      translate(-rot_pt)
        children();
}

module rotate_about_z(angle, point) {
  translate(point)
    rotate(angle)
      translate(-point)
        children();
...
module rotate_about_x(angle, point) {
  translate(point)
    rotate(angle, [1, 0, 0])
      translate(-point)
        children();
...
// Torus function for rounded lip on top nibbles
module torus(major_radius, minor_radius) {
  rotate_extrude($fs=NIBBLE_FS)
    translate([major_radius, 0, 0])
      circle(r=minor_radius, $fs=NIBBLE_FS);
}
...
module doblobottomnibble(height_mm, scale) {
  difference() {
