...
module track (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO) {
	...
	*/
     ...
     track_component (col=col, row=row, up=up+bottom_height, length=length, width=width, closure=closure, orientation=orientation, bottom_height=bottom_height, support_height=support_height, scale=scale);
}

module track_component (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO) {
     /* The track element will be placed along the y axis (back/forth)
	In the doblo system: width= left/right length, length= forth/back length
	...
	*/
     ...
     difference () {
	  ...
	  // dig
	  pipe_rod (col=col, row=row, up=up, width=width, length=length, height_start=0, closure=closure, orientation=orientation, scale=LUGO);
     }
// lay the pipe
...
module slope_component (col=0, row=0, up=0, width=2, length=4, height_start=FULL, support_height=0, closure=5, orientation=0, scale=LUGO) {
     ...
     difference () {
	  ...
	  // dig
	  pipe_rod (col=col, row=row, up=up, width=width, length=length, height_start = height_start, closure=closure, orientation=orientation, scale=LUGO);
     }
     // lay the pipe     
     ...
     difference () {
	  ...
	  translate ([radius, y_0, z_0]) {
	       ...
	       rotate ([90,0,0])
		    ...
		    translate ([0,0,2*NUDGE]) cylinder(h= length*PART_WIDTH(scale)+30*NUDGE, r = radius -TUBE_WALL_WIDTH, center = true, $fs = CYL_FS);
	       }
	  }
...
module curve (col=0, row=
0, up=0, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO, angle = 90) {
...
module curved_tube  (col=0, row=0, up=0, closure=5, orientation=0, angle=90, scale=LUGO, filled=false) {
     ...
     */
     ...
     inner_radius = 1 * PART_WIDTH(scale) - TUBE_WALL_WIDTH;
...
// To do: Add an angle or type argument ?
// curve_wide(angle=180, closure=6, support_height=3, length=12);
// doblo (col=0, row=0, up=3*THIRD, height=THIRD, length=8, width=4, nibbles_on_off=true );
// curve_wide (angle=180);
module curve_wide (col=0, row=0, up=0, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO, angle = 90) {
     /* The curve element will be placed its sharp corner against x=0/y=0 (because this requires the least amount of code)
	In the doblo system: width= left/right length, length= forth/back length
	The tube is embedded in block that sits on a doblo block without nibbles.
	Instead of just digging a hole into a block we create a real tube, allowing for some subtler support structures some day
	...
	*/
     ...
     if (angle == 90 ) {
	  ...
	  color ("red") doblo (col=col, row=row, up=up, width=width, length=length, height=bottom_height, nibbles_on_off=false,diamonds_on_off=false,scale=LUGO,orientation=orientation) ;
	  // Add an tube embedded in a block on top
	  curve_wide_component (col=col, row=row, up=up+bottom_height, length=length, width=width, closure=closure, orientation=orientation, support_height=support_height, scale=scale);
	  }
...
module curve_wide_component (col=0, row=0, up=0, length=4, width=4, closure=5, orientation=0, support_height=0, angle=90, scale=LUGO) {
     ...
     difference () {
	  color("crimson") block (col=col, row=row, up=up, width=width, length=mylength, height=upper_block_h, nibbles_on_off=false, diamonds_on_off=false, scale=LUGO,orientation=orientation) ;
	  // dig the hole
	  curved_wide_tube (col=col, row=row, up=up, length=length, width=width, closure=closure, orientation=orientation,  support_height=support_height, scale=scale, filled=true, angle=angle);
     }
     // Add the pipe, shave off the top
     difference () {
...
module curved_wide_tube  (col=0, row=0, up=0, length=8, width=4, closure=5, orientation=0, angle=90, scale=LUGO, filled=false) {
	...
	- angle should be either 90 or 180. Other angles could be implemented in the future, in particular when normal OpenSCAD distributions accept rotate_extruct(angle= ....)
     ...
     inner_radius = PART_WIDTH(scale) - TUBE_WALL_WIDTH;
...
hole ();
module hole (height=BOTTOM_HEIGHT) {
     // block
     block_component (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO);
     }

module block_component (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO) {
     /* The track element will be placed along the y axis (back/forth)
	In the doblo system: width= left/right length, length= forth/back length
	The tube is embedded in a block that allows creating playmobile like structures, or just printing it.
	Params
     ...
     */
     ...
     difference () {
	  union () {
	       color ("Ghostwhite") doblo (col=2, row=2, height=BOTTOM_HEIGHT, length=2,  width=2, nibbles_on_off=false );
	       color ("SlateGray") block (col=2, row=2, up=BOTTOM_HEIGHT, height=THIRD, length=2,  width=2, nibbles_on_off=false );
	       }
	  tube_vertical();
     }
}
