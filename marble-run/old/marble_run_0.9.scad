...
sloped_curved_tube_component (angle=180, height_start=2*FULL, curve_radius=3, closure=8);
...
module sloped_curved_tube_component (col=0, row=0, up=0, curve_radius=3, closure=5, orientation=0, angle=180, scale=LUGO, filled=false, height_start=FULL, support_height=0, scale=SCALE) {
	...
	The radius of the curved tube is adjusted.
     ...
     // width depends on radius, e.g. curve_radius = 2, gives mywidth = 3 * PART_WIDTH
     mywidth = curve_radius+1;
     ...
     rot_point = [0,-tube_radius,tube_radius];
     
     // prepare the bed
     difference () {
	  union () {
	       color("green") block (col=col, row=row, up=up, width=mywidth, length=mylength, height=upper_block_h, nibbles_on_off=false, diamonds_on_off=false, scale=scale,orientation=orientation,debug=true) ;
	       color ("hotpink") triangle_block (col=col, row=row, up=up+upper_block_h, width=mywidth, length=mylength,
						 height_start = height_start, scale=scale,orientation=orientation) ;
	  }
	  // dig the hole
	  translate ([0,0,  height_start * PART_HEIGHT (scale)])
	       rotate_about_pt (angle, [1,0,0], rot_point)
	       curved_tube (col=col, row=row, up=0, closure=closure, height_start=height_start, orientation=orientation, scale=scale, filled=true, angle=angle, curve_radius=curve_radius_fixed);
     }
     ...
     translate ([0,0,  height_start * PART_HEIGHT (scale)])
	  rotate_about_pt (angle, [1,0,0], rot_point)
	  difference () {
	  curved_tube (col=col, row=row, up=0, closure=closure, height_start=height_start, orientation=orientation, scale=scale, angle=angle,  curve_radius=curve_radius_fixed);
	  // position of the cutting the block for the tube according to closure.
	  top_block_shave (col=col-mywidth, row=row-1, width=mywidth*3, length=mylength*2, up=up+upper_block_h-NUDGE, height=mylength*4, scale=scale);
     }
}
