...
// test raw slope
*/
slope_component (col=-10, row=0, up=0, length=6,  width=2, closure=5, height_start=FULL,orientation=0);
...
module track (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, scale=LUGO) {
	...
	*/
     ...
     track_component (col=col, row=row, up=up+bottom_height, length=length, width=width, closure=closure, orientation=orientation, bottom_height=bottom_height, scale=scale);
}

module track_component (col=0, row=0, up=0, length=4, width=2, closure=5, orientation=0, bottom_height=BOTTOM_HEIGHT, scale=LUGO) {
     /* The track element will be placed along the y axis (back/forth)
	In the doblo system: width= left/right length, length= forth/back length
	The tube is embedded in block and you use it to create playmobile like structures, or just print it 
	Params
	- closure is a value between 2 and 12 and it defines the height of block that will shave off the tube
	- bottom_height is the height of the block underneath the tube, e.g. the lego/duplo compatible
	*/
     ...
     // prepare the bed
     difference () {
	  color("grey") block (col=col, row=row, up=up, width=width, length=length, height=upper_block_h,
			       nibbles_on_off=false, diamonds_on_off=false, scale=LUGO,orientation=orientation) ;
	  // dig
	  pipe_rod (col=col, row=row, up=up, bottom_height=bottom_height, width=width, length=length, height_start=0, closure=closure, orientation=orientation, scale=LUGO);
     }
// lay the pipe
     pipe (col=col, row=row, up=up, bottom_height=bottom_height, width=width, length=length, height_start=0, closure=closure, orientation=orientation, scale=LUGO);
}
...
module slope (col=0, row=0, up=0, width=2, length=4, bottom_height=BOTTOM_HEIGHT, height_start=FULL, closure=3, orientation=0, scale=LUGO) {
     ...
     slope_component (col=col, row=row, up=up+bottom_height, length=length, bottom_height=bottom_height, width=width, closure=closure, orientation=orientation, scale=scale);
}


module slope_component (col=0, row=0, up=0, width=2, length=4, bottom_height=0, height_start=FULL, closure=3, orientation=0, scale=LUGO) {
     // The slope element will be placed along the y axis (back/forth), higher end in back
     // In the doblo system: width= left/right length, length= forth/back length
     // height_start should use Lego scale values, i.e. THIRD, FULL (aka 2,6)
     // closure is a value between 2 and 6
     ...
     difference () {
	  union () {
	       color ("grey") block (col=col, row=row, up=up+bottom_height, width=width, length=length,
				     height=upper_block_h, nibbles_on_off=false, diamonds_on_off=false, scale=LUGO, orientation=orientation) ;
	       triangle_block (col=col, row=row, up=up+bottom_height+upper_block_h, width=width, length=length,
...
module pipe_sloped_0 (length=4, width=2, closure=5, up=0, scale=LUGO, height_start=FULL, scale=SCALE) {
	...
	*/
     ...
     difference () {
	  ...
	  # block (col=-1, up=-2*FULL, row=length, width=width*2, length=2, height=up+3*FULL);
     }
}
