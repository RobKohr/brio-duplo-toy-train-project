...
curved_slope (col=2.5, angle=180, height_start=2*FULL, curve_radius=2, closure=6, orientation=0);
...
module curved_slope (col=0, row=0, up=0, closure=6, orientation=0, bottom_height=BOTTOM_HEIGHT, support_height=0, scale=LUGO, angle = 90, nibbles_on_off=true,  id="some wide sloped curve") {
     ...
     */
     ...
     //compute the length of the block in function of radius
	  ...
	  color ("red") doblo (col=col, row=row, up=up, width=width, length=length, height=bottom_height, nibbles_on_off=false,diamonds_on_off=false,scale=scale,orientation=orientation) ;
	  // Add an tube embedded in a block on top
	  curved_slope_component (col=col, row=row, up=up+bottom_height, curve_radius=curve_radius, closure=closure, orientation=orientation, height_start=height_start, support_height=support_height, scale=scale, angle=180);
       // nibbles
       if (nibbles_on_off) {
...
// curved_slope_component (angle=180, height_start=2*FULL, curve_radius=3, closure=6, orientation=0);
...
// curved_slope_component (col=6, angle=180, height_start=2*FULL, curve_radius=1, closure=6, orientation=0);
// curved_slope_component (angle=90, height_start=2*FULL, curve_radius=2, closure=10, orientation=0);
		      
		      
module curved_slope_component (col=0, row=0, up=0, curve_radius=3, closure=5, orientation=0, angle=180, scale=LUGO, filled=false, height_start=FULL, support_height=0, scale=SCALE) {

     /* Low level construct. A curved_tube tilted down an angle. 
	The radius of the curved tube is adjusted.
	Params:
	- angle is the tube (180 = half) (90 = quarter)
	...
	- Add orientation maybe (a simple transform at top level ?)
     ...
     debug_msg (str ("Sloped_curved_tube, curve_radius=", curve_radius, ",  height_start=", height_start, ", mylength=", mylength, ", mylength_mm=", mylength_mm, ",  hypothenuse_mm=", hypothenuse_mm, ",   curve_radius_mm=", curve_radius_mm, ",  cosecant=", cosecant, ",  curve_radius_fixed=", curve_radius_fixed, ",  horiz. angle=", angle));
     ...
     rot_point = (angle==180) ? [0,-tube_radius_mm,tube_radius_mm] : [0,0,0];
	  ...
	  union () {
	       difference () {
		    union () {
			 ...
			 color("green") block (col=col, row=row, up=up, width=mywidth, length=mylength, height=upper_block_h_fixed, nibbles_on_off=false, diamonds_on_off=false, scale=scale,orientation=0,debug=true) ;
