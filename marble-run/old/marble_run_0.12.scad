/* Marble run kit, Lego and duplo compatible
   ...
   You will have to find 1cm marbles for Lego compatibles. I recommend 10mm steel balls (about 1$/€ per ball). Marble balls are usually bigger (1.6mm) and only fit Duplo compatibles.
...
// Overrides
LATTICE_TYPE   = 1;  // type 3 extends lattice to bottom (more solid, but may need force to fit)
...
// ball
// To do: Add an appropriate ring around the base to stabilize the print

marble_ball ();
module marble_ball () {
     r = 10;  // 2cm ball. Print with skirt that has several layers and good filling.
     translate ([0,0,r])
...
// track (scale=DOBLO);
// slope (col=0, row=0, up=0, length=8,  width=2, closure=5, height_start=FULL,orientation=0, scale = DOBLO);
	  ...
	  union () {
	       difference () {
		    union () {
			 ...
			 // triangle on top (@@@ Fix upper_block_h !!!
