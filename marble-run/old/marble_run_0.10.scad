...
module stairways_scale () {
    doblo   (4,   7,   0,   2,   1,   0,  true, false, scale=SCALE, id="10" );
    doblo   (4,   6,   0,   2,   1,   THIRD,  true, false, scale=SCALE, id="12" );
    doblo   (4,   5,   0,   2,   1,   2*THIRD,  true, false, scale=SCALE, id="13" );
     color ("red") doblo   (4,   4,   0,   2,   1,   FULL,  true, false, scale=SCALE, id="14" );
     doblo   (4,   3,   0,   2,   1,   FULL+THIRD,  true, false, scale=SCALE,  id="15" );
     doblo   (4,   2,   0,   2,   1,   FULL+2*THIRD,  true, false, scale=SCALE,  id="16" );
     color ("red") doblo   (4,   1,   0,   2,   1,   2*FULL,  true, false, scale=SCALE,  id="17" );
     doblo   (4,   0,   0,   2,   1,   2*FULL+THIRD,  true, false, scale=SCALE,  id="18" );
}
...
module curve_component (col=0, row=0, up=0, curve_radius=3, closure=5, orientation=0, support_height=0, angle=90, scale=LUGO, angle=180) {
...
/* -------------------------------------- Sloped Curved tube (low-level)
   @@ not very safe code
   if height_start=0 it also creates flat curved tubes.
 */

// sloped_curved_tube_component (angle=180, height_start=2*FULL, curve_radius=3, closure=6);
sloped_curved_tube_component (angle=180, height_start=1*FULL, curve_radius=1, closure=5);
sloped_curved_tube_component (col=10, angle=180, height_start=3*FULL, curve_radius=3, closure=8);
sloped_curved_tube_component (up=20, col=-6, angle=180, height_start=2*FULL, curve_radius=2, closure=5);
sloped_curved_tube_component (up=0, col=-6, angle=180, height_start=2*FULL, curve_radius=2, closure=5);

module sloped_curved_tube_component (col=0, row=0, up=0, curve_radius=3, closure=5, orientation=0, angle=180, scale=LUGO, filled=false, height_start=FULL, support_height=0, scale=SCALE) {
	...
	The radius of the curved tube is adjusted.
     ...
     debug_msg (str ("Sloped_curved_tube, curve_radius=", curve_radius, ",  height_start=", height_start, ", mylength=", mylength, ", mylength_mm=", mylength_mm, ",  hypothenuse_mm=", hypothenuse_mm, ",   curve_radius_mm=", curve_radius_mm, ",  cosecant=", cosecant, ",  curve_radius_fixed=", curve_radius_fixed, ",  angle=", angle));
     
     angle = atan2(height_start * PART_HEIGHT(scale), mylength * PART_WIDTH(scale)) ;
     ...
     height_fixed = tan(angle) * (mylength_mm_fixed - tube_radius_mm);
     height_fixed = tan(angle) * (mylength_mm - tube_radius_mm*2); // almost works !!
	  ...
	  union () {
	       difference () {
		    union () {
			 ...
			 color("green") block (col=col, row=row, up=up, width=mywidth, length=mylength, height=upper_block_h_fixed, nibbles_on_off=false, diamonds_on_off=false, scale=scale,orientation=orientation,debug=true) ;
			 // triangle on top (@@@ Fix upploer_block_h !!!
			 color ("hotpink") triangle_block (col=col, row=row, up=up+upper_block_h_fixed, width=mywidth, length=mylength, height_start = height_start, scale=scale,orientation=orientation) ;
		    }
		    // dig the hole
		    // translate ([0,0,  height_start * PART_HEIGHT (scale)])
		    translate ([0,0,  up+height_fixed])
			 rotate_about_pt (angle, [1,0,0], rot_point)
			 curved_tube (col=col, row=row, up=0, orientation=orientation, scale=scale, filled=true, angle=angle, curve_radius=curve_radius_fixed);
	       }
	       // Add the pipe, shave off the top
	       translate ([0,0,  up +height_fixed])
		    // translate ([0,0,  height_start * PART_HEIGHT (scale)])
		    rotate_about_pt (angle, [1,0,0], rot_point)
		    curved_tube (col=col, row=row, up=0, orientation=orientation, scale=scale, angle=angle,  curve_radius=curve_radius_fixed);
	  }
	  translate ([0,0, up+ height_fixed])
	       // translate ([0,0,  height_start * PART_HEIGHT (scale)])
	       rotate_about_pt (angle, [1,0,0], rot_point)
	       // position of the cutting the block for the tube according to closure.
	       top_block_shave (col=col-mywidth, row=row-1, width=mywidth*3, length=mylength*2, up=up+upper_block_h, height=mylength*4, scale=scale);
     }
}
...
module curved_tube  (col=0, row=0, up=0, curve_radius=3, orientation=0, angle=90, scale=LUGO, filled=false) {
